...
export function AiPage() {
  ...
  const loadHistory = useCallback(async () => {
    ...
    try {
      const list = await aiChatsDb.list(user.$id, selectedBaby.$id);
      setChats(list);
    } catch {
  ...
  const saveChat = useCallback(async (msgs: Message[], chatId: string | null, firstUserText: string) => {
    if (!user || !selectedBaby || msgs.length === 0) return chatId;
    const storable = msgs.slice(-MAX_MESSAGES_STORED).map(({ id, role, text, isError, retryText }) => ({ id, role, text, isError, retryText }));
    const json = JSON.stringify(storable);
    const title = firstUserText.slice(0, 80) || 'Chat';
    try {
      if (chatId) {
        await aiChatsDb.update(chatId, title, json);
        return chatId;
      } else {
        const doc = await aiChatsDb.create(user.$id, selectedBaby.$id, title, json);
        setChats((prev) => [doc, ...prev.filter((c) => c.$id !== doc.$id)]);
        return doc.$id;
      }
    } catch {
      return chatId;
  ...
  const scheduleAutoSave = (msgs: Message[], chatId: string | null, firstUserText: string) => {
    if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(async () => {
      const newId = await saveChat(msgs, chatId, firstUserText);
      if (newId && newId !== chatId) setCurrentChatId(newId);
    }, 1500);
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (remaining <= 0) { setFileError(`Maximum ${MAX_FILES} files per message.`); e.target.value = ''; return; }
    const allowed = ['image/jpeg', 'image/png', 'image/webp', 'image/gif', 'application/pdf'];
    const toAdd: PendingFile[] = [];
    for (const file of incoming.slice(0, remaining)) {
      if (file.size > MAX_FILE_BYTES) { setFileError(`"${file.name}" is too large (max 4 MB).`); continue; }
      if (!allowed.includes(file.type)) { setFileError(`"${file.name}" is not supported.`); continue; }
      toAdd.push({ file, previewUrl: file.type.startsWith('image/') ? URL.createObjectURL(file) : undefined });
    }
    if (toAdd.length) setPendingFiles((prev) => [...prev, ...toAdd]);
    e.target.value = '';
  };

  const removeFile = (idx: number) => setPendingFiles((prev) => { const c = [...prev]; c.splice(idx, 1); return c; });
  const clearFiles = () => { setPendingFiles([]); setFileError(''); if (fileRef.current) fileRef.current.value = ''; };
  ...
  const send = async (text: string, files: PendingFile[]) => {
    ...
    const userMsg: Message = { id: Date.now().toString(), role: 'user', text: text.trim(), attachments: attachments.length > 0 ? attachments : undefined };
    const historySnapshot = messages.map((m) => ({ role: m.role, text: m.text }));
    const firstUserText = messages.find((m) => m.role === 'user')?.text || text.trim();
    ...
    try {
      const encodedFiles = await Promise.all(files.map(async (pf) => {
        if (pf.file.type.startsWith('image/')) return { fileBase64: await compressImageToBase64(pf.file), fileMediaType: 'image/jpeg', fileName: pf.file.name };
      ...
      const aiMsg: Message = { id: Date.now().toString() + '_ai', role: 'assistant', text: reply };
      const finalMessages = [...nextMessages, aiMsg];
      setMessages(finalMessages);
      scheduleAutoSave(finalMessages, currentChatId, firstUserText);
    } catch (err: any) {
      const message = err?.message === 'routing' || !err?.message
        ...
        : err.message;
      setMessages((prev) => [...prev, { id: Date.now().toString() + '_err', role: 'assistant', text: message, isError: true, retryText: text }]);
    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); send(input, pendingFiles); };
  const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(input, pendingFiles); } };
  ...
  const handleEdit = (msg: Message) => {
    ...
    setTimeout(() => {
      ...
      if (textRef.current) { textRef.current.style.height = 'auto'; textRef.current.style.height = Math.min(textRef.current.scrollHeight, 120) + 'px'; }
    }, 0);
  };
  ...
  const loadChat = (chat: AiChat) => {
    try {
      const msgs: Message[] = JSON.parse(chat.messages);
      setMessages(msgs);
                    ...
                    <div className={`rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                      ...
                      {msg.isError && msg.retryText ? (
                        <div className="flex flex-col gap-2">
                          <p>{msg.text}</p>
                          <button onClick={() => send(msg.retryText!, [])} disabled={loading} className="flex items-center gap-1.5 text-xs font-semibold text-red-600 dark:text-red-400 hover:text-red-700 disabled:opacity-50 self-start">
                ...
                <div key={idx} className="relative flex-shrink-0 w-16 h-16 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
                  {pf.previewUrl ? <img src={pf.previewUrl} alt="Preview" className="w-full h-full object-cover" /> : (
                    <div className="w-full h-full flex flex-col items-center justify-center gap-1 px-1">
                      ...
                      <span className="text-[9px] text-gray-500 truncate w-full text-center leading-tight">{pf.file.name}</span>
                    </div>
                  )}
                  <button onClick={() => removeFile(idx)} className="absolute top-0.5 right-0.5 w-5 h-5 rounded-full bg-gray-900/60 text-white flex items-center justify-center hover:bg-red-500/80 transition-colors">
                    <X size={10} />
                  </button>
                </div>
              ))}
                    ...
                    {chats.map((chat) => (
                      <motion.button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-left transition-colors ${
                          currentChatId === chat.$id
                            ? 'bg-brand-light dark:bg-brand-dark/20 border border-brand-mint'
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400">{formatChatDate(chat.$updatedAt)}</p>
                        </div>
                        <button
