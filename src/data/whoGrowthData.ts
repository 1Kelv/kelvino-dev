// WHO 2006 Child Growth Standards — weight-for-age reference data (kg), 0–24 months

export interface WHOWeightDataPoint {
  month: number;
  p3: number;
  p15: number;
  p50: number;
  p85: number;
  p97: number;
}

export const whoBoysWeight: WHOWeightDataPoint[] = [
  { month: 0,  p3: 2.5,  p15: 2.9,  p50: 3.3,  p85: 3.9,  p97: 4.3  },
  { month: 1,  p3: 3.4,  p15: 3.9,  p50: 4.5,  p85: 5.1,  p97: 5.7  },
  { month: 2,  p3: 4.3,  p15: 4.9,  p50: 5.6,  p85: 6.3,  p97: 7.1  },
  { month: 3,  p3: 5.0,  p15: 5.7,  p50: 6.4,  p85: 7.2,  p97: 8.0  },
  { month: 4,  p3: 5.6,  p15: 6.2,  p50: 7.0,  p85: 7.8,  p97: 8.7  },
  { month: 5,  p3: 6.0,  p15: 6.7,  p50: 7.5,  p85: 8.4,  p97: 9.3  },
  { month: 6,  p3: 6.4,  p15: 7.1,  p50: 7.9,  p85: 8.8,  p97: 9.8  },
  { month: 7,  p3: 6.7,  p15: 7.4,  p50: 8.3,  p85: 9.2,  p97: 10.2 },
  { month: 8,  p3: 6.9,  p15: 7.7,  p50: 8.6,  p85: 9.6,  p97: 10.5 },
  { month: 9,  p3: 7.1,  p15: 7.9,  p50: 8.9,  p85: 9.9,  p97: 10.9 },
  { month: 10, p3: 7.4,  p15: 8.2,  p50: 9.2,  p85: 10.2, p97: 11.2 },
  { month: 11, p3: 7.6,  p15: 8.4,  p50: 9.4,  p85: 10.5, p97: 11.5 },
  { month: 12, p3: 7.7,  p15: 8.6,  p50: 9.6,  p85: 10.8, p97: 11.8 },
  { month: 13, p3: 7.9,  p15: 8.8,  p50: 9.9,  p85: 11.0, p97: 12.1 },
  { month: 14, p3: 8.1,  p15: 9.0,  p50: 10.1, p85: 11.3, p97: 12.4 },
  { month: 15, p3: 8.3,  p15: 9.2,  p50: 10.3, p85: 11.5, p97: 12.6 },
  { month: 16, p3: 8.4,  p15: 9.4,  p50: 10.5, p85: 11.7, p97: 12.9 },
  { month: 17, p3: 8.6,  p15: 9.6,  p50: 10.7, p85: 11.9, p97: 13.1 },
  { month: 18, p3: 8.8,  p15: 9.8,  p50: 10.9, p85: 12.2, p97: 13.4 },
  { month: 19, p3: 8.9,  p15: 10.0, p50: 11.1, p85: 12.4, p97: 13.6 },
  { month: 20, p3: 9.1,  p15: 10.1, p50: 11.3, p85: 12.6, p97: 13.9 },
  { month: 21, p3: 9.2,  p15: 10.3, p50: 11.5, p85: 12.8, p97: 14.1 },
  { month: 22, p3: 9.4,  p15: 10.5, p50: 11.8, p85: 13.1, p97: 14.4 },
  { month: 23, p3: 9.5,  p15: 10.7, p50: 12.0, p85: 13.4, p97: 14.7 },
  { month: 24, p3: 9.7,  p15: 10.8, p50: 12.2, p85: 13.6, p97: 15.0 },
];

export const whoGirlsWeight: WHOWeightDataPoint[] = [
  { month: 0,  p3: 2.4,  p15: 2.8,  p50: 3.2,  p85: 3.7,  p97: 4.2  },
  { month: 1,  p3: 3.2,  p15: 3.6,  p50: 4.2,  p85: 4.8,  p97: 5.5  },
  { month: 2,  p3: 3.9,  p15: 4.5,  p50: 5.1,  p85: 5.8,  p97: 6.6  },
  { month: 3,  p3: 4.5,  p15: 5.2,  p50: 5.8,  p85: 6.6,  p97: 7.5  },
  { month: 4,  p3: 5.0,  p15: 5.7,  p50: 6.4,  p85: 7.3,  p97: 8.2  },
  { month: 5,  p3: 5.4,  p15: 6.1,  p50: 6.9,  p85: 7.8,  p97: 8.8  },
  { month: 6,  p3: 5.7,  p15: 6.5,  p50: 7.3,  p85: 8.2,  p97: 9.3  },
  { month: 7,  p3: 6.0,  p15: 6.8,  p50: 7.6,  p85: 8.6,  p97: 9.7  },
  { month: 8,  p3: 6.3,  p15: 7.0,  p50: 7.9,  p85: 9.0,  p97: 10.1 },
  { month: 9,  p3: 6.5,  p15: 7.3,  p50: 8.2,  p85: 9.3,  p97: 10.5 },
  { month: 10, p3: 6.7,  p15: 7.5,  p50: 8.5,  p85: 9.6,  p97: 10.9 },
  { month: 11, p3: 6.9,  p15: 7.7,  p50: 8.7,  p85: 9.9,  p97: 11.2 },
  { month: 12, p3: 7.0,  p15: 7.9,  p50: 8.9,  p85: 10.1, p97: 11.5 },
  { month: 13, p3: 7.2,  p15: 8.1,  p50: 9.2,  p85: 10.4, p97: 11.8 },
  { month: 14, p3: 7.4,  p15: 8.3,  p50: 9.4,  p85: 10.7, p97: 12.1 },
  { month: 15, p3: 7.6,  p15: 8.5,  p50: 9.6,  p85: 11.0, p97: 12.4 },
  { month: 16, p3: 7.7,  p15: 8.7,  p50: 9.8,  p85: 11.2, p97: 12.7 },
  { month: 17, p3: 7.9,  p15: 8.9,  p50: 10.0, p85: 11.4, p97: 13.0 },
  { month: 18, p3: 8.1,  p15: 9.1,  p50: 10.2, p85: 11.7, p97: 13.2 },
  { month: 19, p3: 8.2,  p15: 9.2,  p50: 10.4, p85: 11.9, p97: 13.5 },
  { month: 20, p3: 8.4,  p15: 9.4,  p50: 10.6, p85: 12.1, p97: 13.7 },
  { month: 21, p3: 8.6,  p15: 9.6,  p50: 10.9, p85: 12.4, p97: 14.0 },
  { month: 22, p3: 8.7,  p15: 9.8,  p50: 11.1, p85: 12.6, p97: 14.3 },
  { month: 23, p3: 8.9,  p15: 10.0, p50: 11.3, p85: 12.9, p97: 14.6 },
  { month: 24, p3: 9.0,  p15: 10.2, p50: 11.5, p85: 13.2, p97: 14.8 },
];

export function getWHOWeightData(gender?: 'male' | 'female' | 'other' | null): WHOWeightDataPoint[] {
  return gender === 'female' ? whoGirlsWeight : whoBoysWeight;
}
